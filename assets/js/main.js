...
(function () {
  ...
  // =========================================================================
  // 11. Website Traffic Telemetry (Real Logging to Supabase)
  // =========================================================================
  async function initTrafficReport() {
    // Only log 1 real visit per browser session to Supabase
    var isNewSession = !sessionStorage.getItem('toan_traffic_logged_session');
    if (isNewSession && window.supabase && SUPABASE_CONFIG.url) {
      try {
        ...
        console.warn('Supabase telemetry note:', sbErr);
      }
    }
  }
